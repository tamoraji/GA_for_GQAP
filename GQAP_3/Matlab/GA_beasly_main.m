...
for iter = 1:maxiter
    ...
    fitness_P0 = costcalc(P0,F, D,A,T);
    disp('overall fitness of previous generation is:')
    disp(sum(fitness_P0))
    [~,unfitness_P0] = unfitness_calc(P0,C,R);
    disp('overall unfitness of previous generation is:')
    ...
    if check_duplicate
        ...
        disp('new overall fitness is:')
        disp(sum(fitness_P1))
        [~,unfitness_P1] = unfitness_calc(P1,C,R);
        disp('new overall unfitness is')
