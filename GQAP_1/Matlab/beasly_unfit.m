function [child, fit_child,unfitness_child] = beasly_unfit(child,C,R,F,D,A)
...
if unfitness_child >0
    ...
    for k=1:num_overused
        mc = find(child(:) == over_used_loc(k)); %find all the m/c's that is assigned to the overused loc
    ...
    fit_child = costcalc_B(child,F,D,A);
    ...
    disp('no unfitness')
    fit_child = costcalc(child,F,D,A);
end
end